...
class UserInterface :
    def __init__(self,controller:Controller):
        ...
        #แสดงคะแนนสอบ
        self.scoreLabel = Label(text="คะแนน : 0",fg='black',bg=THEME_APP)
        self.scoreLabel.grid(row=0,column=2)
        #แสดงคำถาม
        self.canvas=Canvas(width=300,height=250,bg='white')
        self.question_text=self.canvas.create_text(150,125,width=280,text="ploy",font=('Arial',15,'bold'),fill=THEME_APP)
        self.canvas.grid(row=1,column=1,columnspan=2,pady=50)
        #ปุ่มให้เลือก True/Flase
        true_image = PhotoImage(file="images/images/check.png")
        self.true_button = Button(image=true_image,command=self.true_pressed)
        self.true_button.grid(row=2,column=1)   

        false_image = PhotoImage(file="images/images/remove.png")
